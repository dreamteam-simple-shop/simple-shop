...
function FetchCartItem({ id, cartTotal }) {
  ...
  return product ? <CartItem title={product.title} thumbnail={product.thumbnail} price={product.price} category={product.category} cartTotal={cartTotal} /> : null;
}
...
function CartItems({ isOpen, setIsOpen, cartItems, cartTotal }) {
  return (
    <div className={`fixed top-0 right-0 min-w-[400px] w-1/3 h-screen bg-white z-50 p-4 flex flex-col gap-5 transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "translate-x-full"}`} onClick={(e) => e.stopPropagation()}>
      ...
      <h1 className="text-sm font-medium w-full text-right border-t border-gray-200 mt-auto pt-4 self-end">Total: ${cartTotal.toFixed(2)}</h1>
...
export default function ShoppingCart({ isOpen, setIsOpen }) {
  ...
  return (
    <>
      ...
      <CartItems isOpen={isOpen} setIsOpen={setIsOpen} cartItems={cartItems} cartTotal={cartTotal} />
    </>
  );
}
