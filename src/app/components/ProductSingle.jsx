			...
			<div className="md:col-1 ">
				<figure className="grid justify-stretch self-center pt-5">
					<Image
						...
						width={400}
						height={400}
						className="col-1 row-1 rounded-4xl self-stretch object-cover aspect-square"
					></Image>
					<div className="col-1 row-1 justify-self-start p-3">
						{/* <p className="bg-black text-white py-1 px-2 rounded-xl m-1">New</p> */}
						<p
							className={`bg-red-600 text-white py-1 px-2 rounded-xl m-1 ${discountPercentage > 10 ? 'block' : 'hidden'}`}
						>
							Sale
						</p>
					</div>
				...
				<div>
					<div className=" grid gap-2 pb-4">
						...
						<div className="flex gap-5 items-center">
							...
							<h2
								className={`text-2xl text-gray-400 line-through ${discountPercentage > 10 ? 'block' : 'hidden'}`}
							>
								$ {price.toFixed(2)}
							</h2>
							<p
								className={`bg-red-600 text-white py-1 px-3 rounded-xl ${discountPercentage > 10 ? 'block' : 'hidden'}`}
							>
					...
					<div className="inline-grid grid-cols-3 gap-3">
						{sizes.map((size) => (
							<button
								...
								className={
									selectedSize === size
										? 'bg-black text-white w-15 h-15 rounded-2xl'
										: 'border border-black w-15 h-15 rounded-2xl'
								}
							>
								{size}
							</button>
				...
				<div className="self-start">
					...
					<div className="flex border border-gray-400 rounded-xl  items-center p-5 justify-self-start">
						...
						<input
							...
							value={isQuant}
							min={0}
							onChange={(e) => setIsQuant(Math.max(0, Number(e.target.value)))}
						/>
						<button
							onClick={() => {
