...
function ProductCard({
	...
	return (
		<Link
			href={`/detalje/${id}`}
			key={key}
			className="rounded-4xl shadow-sm gap-2 grid grid-rows-auto"
		>
			<figure className="grid">
				<Image
					...
					width={800}
					height={800}
					className="col-1 row-1 rounded-t-4xl self-stretch object-cover aspect-square"
				></Image>
				<div className="col-1 row-1 justify-self-start p-3">
					{/* <p className="bg-black text-white text-xs py-1 px-2 rounded-xl m-1">New</p> */}
					<p
						className={`bg-red-600 text-white text-xs py-1 px-2 rounded-xl m-1 ${discountPercentage > 10 ? 'block' : 'hidden'}`}
					>
						Sale
					</p>
				</div>
					...
					<div>
						...
						<p>
							...
							<span className="line-through text-gray-400">
								{discountPercentage > 10 ? '$ ' + price.toFixed(2) : ''}
							</span>
						</p>
					</div>
				</div>
